"""Small non-notebook TMz dispersive metasurface workflow.

This file can be launched directly from any working directory.  It prefers the
repository's local ``src/fdtdz_jax`` tree, so an editable package installation
is not required while developing the project.
"""
...
from fdtdz_jax import run_tmz_scattering_2d
...
result = run_tmz_scattering_2d(
    materials, reference_ids, device_ids, dx, dy, dt, waveform,
